'''
@author: Yiz56865
'''

import boto.ec2
import sys
import time

# specify AWS keys
auth = {"aws_access_key_id": "xxxx", "aws_secret_access_key": "xxxx"}
region= "ap-northeast-1"
instance = "i-05c310e6e1dabfe39"

def main():
    # read arguments from the command line and 
    # check whether at least two elements were entered
    if len(sys.argv) < 2:
        print("Usage: python aws.py {start|stop}\n")
        sys.exit(0)    
    else:
        action = sys.argv[1] 

    if action == "start":
        startInstance()
    elif action == "stop":
        stopInstance()
    elif action == "status":
        statusInstance()
    else:
        print("Usage: python aws.py {start|stop|status}\n")

def startInstance():
    print "Starting the instance..."

    # change "eu-west-1" region if different
    try:
        ec2 = boto.ec2.connect_to_region(region, **auth)

    except Exception, e1:
        error1 = "Error1: %s" % str(e1)
        print(error1)
        sys.exit(0)

    # change instance ID appropriately  
    try:
         ec2.start_instances(instance_ids=instance)

    except Exception, e2:
        error2 = "Error2: %s" % str(e2)
        print(error2)
        sys.exit(0)

def stopInstance():
    print "Stopping the instance..."

    try:
        ec2 = boto.ec2.connect_to_region(region, **auth)

    except Exception, e1:
        error1 = "Error1: %s" % str(e1)
        print(error1)
        sys.exit(0)

    try:
         ec2.stop_instances(instance_ids=instance)

    except Exception, e2:
        error2 = "Error2: %s" % str(e2)
        print(error2)
        sys.exit(0)

def statusInstance():
    print "Get the instance status..."
    
    try:
        ec2 = boto.ec2.connect_to_region(region, **auth)

    except Exception, e1:
        error1 = "Error1: %s" % str(e1)
        print(error1)
        sys.exit(0)
        
    try:
         print type(ec2.get_only_instances(instance_ids=[instance,])[0])
         
         vpc = ec2.get_only_instances(instance_ids=[instance,])[0]
         while vpc.update() != "running":
             time.sleep(5)
             print vpc.update()
             
         print vpc.ip_address
         
    except Exception, e2:
        error2 = "Error2: %s" % str(e2)
        print(error2)
        sys.exit(0)            


if __name__ == '__main__':
    main()